...
let getUserMoneyDetail = function(userId) {
  return fetch(api.GET_USER_MONEY_DETAIL, {
    userId
  })
}
...
let getRefundReasons = function(userId) {
  return fetch(api.GET_REFUND_REASONS, {
    userId
  })
}
...
let getRefundCondition = function(userId, amount, userMoneyType) {
  return fetch(api.GET_REFUND_CONDITION, {
    userId,
    amount,
    userMoneyType
  })
...
let getSesameAuth = function(userId){
  return fetch(api.GET_SESAME_AUTH, {
    userId
  })
}
