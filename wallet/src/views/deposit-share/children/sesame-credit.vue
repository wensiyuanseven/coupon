...
<div class="sesame-credit">
  ...
  <mt-button type="primary" @click="getSesameAuth">去授权</mt-button>
...
export default {
  ...
  methods: {
    getSesameAuth(){
      dataService.getSesameAuth(this.$store.getters.user.id).then(res => {
        this.$router.replace({
          name: 'sesameAuth',
          query: {
  ...
  created(){
    dataService.getSesameConfig().then(res => {
      let result = res.data
